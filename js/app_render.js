...
function renderRoleSelector(){
  ...
  // 몬타주 종료 직후 첫 렌더에서만 로그인 2초 페이드 인 — 이후 재렌더(비번 모달 등)에선 재생 안 함
  var reveal = !introing && heroImgs.length>1 && !window.__heroLoginRevealed;
  ...
  root.innerHTML=`<div class="role-hero${introing?' introing':(reveal?' reveal':'')}">
  ...
  </div>${S.showPwModal?'<div class="modal-overlay" onclick="if(event.target===this)cancelPassword()"><div class="modal pw-modal" style="width:340px"><div class="modal-title" style="text-align:center">🔒 '+(S.pendingRole?S.pendingRole.user:'')+'</div>'+(bio.available&&S.pendingRole&&bio.isRegistered(S.pendingRole.role,S.pendingRole.user)?'<button class="btn bio-btn"'+(S.bioBusy?' disabled':'')+' onclick="bioLoginNow()">'+(S.bioBusy?'🔓 인증 중...':'🆔 Face ID · 지문으로 로그인')+'</button><div class="pw-divider"><span>또는 비밀번호</span></div>':'')+'<div class="form-group"><label class="form-label">비밀번호</label><input class="form-input" type="password" placeholder="비밀번호를 입력하세요" oninput="S.pwInput=this.value" onkeydown="if(event.key===\'Enter\')submitPassword()" autofocus></div>'+(S.pwError?'<div style="color:#993c1d;font-size:12px;margin-bottom:10px;text-align:center">비밀번호가 일치하지 않습니다</div>':'')+(S.bioError?'<div style="color:#993c1d;font-size:11.5px;margin-bottom:10px;text-align:center">'+S.bioError+'</div>':'')+(bio.available&&S.pendingRole&&bio.isRegistered(S.pendingRole.role,S.pendingRole.user)&&!S.bioBusy?'<div style="text-align:center;margin:-4px 0 10px"><a href="#" onclick="event.preventDefault();bioReenrollHint()" style="font-size:11.5px;color:var(--tx-3);text-decoration:underline">생체 로그인이 안 되나요? 지문·Face ID 다시 등록</a></div>':'')+(!bio.available?'<label class="pw-trust"><input type="checkbox" '+(S.trustDevice?'checked':'')+' onchange="S.trustDevice=this.checked"><span>이 기기에서 자동 로그인<small>다음부터 비밀번호 없이 바로 입장 (스튜디오 공용 기기용)</small></span></label>':'')+'<div class="modal-actions"><button class="btn" onclick="cancelPassword()">취소</button><button class="btn primary" onclick="submitPassword()">확인</button></div></div></div>':''}${S.bioEnrollFor?'<div class="modal-overlay"><div class="modal" style="width:360px;text-align:center"><div style="font-size:46px;margin:6px 0 10px">🆔</div><div class="modal-title" style="text-align:center;margin-bottom:8px">'+(S.bioEnrollFor.reenroll?'생체 로그인 다시 등록':'생체 로그인 등록')+'</div><div style="font-size:13px;color:var(--tx-2);line-height:1.7;margin-bottom:16px">'+(S.bioEnrollFor.reenroll?'이 기기의 생체 로그인 등록이 풀려 있었어요<br><span style="font-size:11px;color:var(--tx-3)">(폰 업데이트·지문 재등록 뒤에 흔한 일 · 다시 등록하면 바로 복구)</span><br>':'')+'이 기기에서 다음부터<br><b>Face ID / 지문 / 홍채</b>로 즉시 로그인할 수 있어요.<br><span style="font-size:11px;color:var(--tx-3)">(이 기기에만 저장 · 서버 전송 없음)</span></div>'+(S.bioError?'<div style="color:#993c1d;font-size:11.5px;margin-bottom:10px">'+S.bioError+'</div>':'')+'<div class="modal-actions" style="justify-content:center;gap:8px"><button class="btn" onclick="bioEnrollSkip()">다음에</button><button class="btn primary"'+(S.bioBusy?' disabled':'')+' onclick="bioEnrollNow()">'+(S.bioBusy?'등록 중...':'🆔 등록하기')+'</button></div></div></div>':''}`;
  ...
  if(introing){
    ...
    if(window.__heroIntro && !window.__heroIntro.done){
      ...
      first.src=heroImgs[0];
      (function waitSplash(){
        if(window.__heroIntro!==intro||intro.done) return;
        if(!document.getElementById('app-splash')){ splashGone=true; startClock(); return; }
        setTimeout(waitSplash, 100);
      })();
      // 안전망 — 5초가 지나면 조건과 무관하게 시작 (사진 URL 불량·스플래시 잔존 대비)
      setTimeout(function(){ imgReady=true; splashGone=true; startClock(); }, 5000);
      // 나머지 사진도 미리 받아둬서 페이드 시점에 공백이 없게
      for(var pi=1;pi<heroImgs.length;pi++){ var pim=new Image(); pim.src=heroImgs[pi]; }
    }
  } else {
...
function _render(){
  ...
  <div class="sidebar${S.sidebarOpen?' open':''}">
    ...
    <div class="infodesk-tools">
      ...
      <div class="infodesk-summary">${(function(){var mm=S.members.filter(function(m){return !m.ownerWatch;});return 'PT+골프 '+mm.filter(function(m){return (m.memberType||'pt_lesson')==='pt_lesson';}).length+'명 · 골프 '+mm.filter(function(m){return m.memberType==='lesson';}).length+'명 · 총 '+mm.length+'명';})()}</div>
    </div>
    ` : `
    ...
    <div class="sidebar-mypage">
      ...
      ${S.currentRole==='admin'?'<button class="mp-btn" onclick="testAiConnection()">🤖 AI 정리 연결 테스트</button>':''}
  ...
  <div class="main">
    ...
    <div class="topbar">
      ...
      <div class="topbar-actions">
        ...
        ${(S.currentRole==='pro'||S.currentRole==='trainer')?'<button class="btn primary live-mini" onclick="openLiveForMember(\''+mid+'\')" title="타석에서 트랙맨 샷을 자동 저장하며 실시간 레슨">🏌️ 타석 레슨</button>':''}
        ${S.currentRole==='pro'?'<button class="btn journal-btn" onclick="openAddSession()" title="샷 저장 없이 일지만 작성">✏️ 일지만 기록</button>':''}
        ${S.currentRole==='trainer'?'<button class="btn journal-btn" onclick="openAddSession()" title="PT 레슨일지 작성">📝 피티레슨일지</button>':''}
            ...
            <div class="session-card${so?' open':''}${(so&&S._animSession===s.id)?' just-opened':''}">
              ...
              <div class="session-bd">
                ...
                <div class="session-actions">
                  ${(isAdmin||!isInfo)?'<button class="small-btn edit" onclick="openEditSession(\''+s.id+'\')">'+'수정</button>':''}
                  ${(isAdmin||!isInfo)?'<button class="small-btn del" onclick="deleteSession(\''+s.id+'\')">'+'삭제</button>':''}
                </div>
              </div>
            </div>`; }).join('')}
        </div>
  ...
  <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
    <div class="modal">
      ...
      ${(!S.newSession._aiPending&&S.newSession._aiItems&&S.newSession._aiItems.length)?'<div class="ai-items"><div class="ai-items-h">🤖 AI가 정리한 항목 <small>체크한 것만 일지에 담겨요 · 핵심 2~3개 권장</small></div>'+S.newSession._aiItems.map(function(it,i){return '<label class="ai-item'+(it.on?' on':'')+'"><input type="checkbox" '+(it.on?'checked':'')+' onchange="toggleAiItem('+i+')"><span>'+(it.tag&&it.tag!=='📋'?it.tag+' ':'')+String(it.t).replace(/</g,'&lt;').replace(/\*\*/g,'')+'</span></label>';}).join('')+'</div>':''}
