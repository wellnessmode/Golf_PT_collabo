...
function _render(){
  ...
  <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
    <div class="modal">
      ...
      ${S.newSession._aiPending?'<div class="ai-pending">🤖 AI가 레슨 내용을 정리하는 중... (몇 초) — 완료 후 저장하는 것을 권장</div>':''}
      ...
      ${(S.editSessionId&&!S.newSession._aiPending&&S.newSession.rawTranscript&&S.newSession.rawTranscript.trim())?'<div class="ai-pending" style="background:#eef6ff;color:#1a4a7a">🎙 녹음 원문이 보관돼 있습니다. 급하게 저장돼 조각으로 남은 일지를 형식 있는 일지로 되살릴 수 있어요. <button type="button" class="ex-add-btn" onclick="if(confirm(\'현재 내용을 AI 정리 결과로 교체합니다. 계속할까요?\'))retryAiSummarize()">🤖 AI로 다시 정리</button></div>':''}
