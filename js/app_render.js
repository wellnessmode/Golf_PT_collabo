          ...
          sessions.map(s => `
            <div class="session-card">
              ...
              <div class="session-bd">
                ...
                ${s.media&&s.media.length>0?'<div class="session-media">'+s.media.map(function(m,mi){var localSrc=m.mediaId?(S.mediaUrls[m.mediaId]||''):'';var remoteSrc=(r2.enabled&&(m.r2Key||m.mediaId)&&m.r2Status!=='failed')?r2.url(m.r2Key||m.mediaId):'';var src=localSrc||remoteSrc||(m.data||'');var mime=m.mimeType||(m.data||'').slice(5,30)||'';var isImg=mime.indexOf('image/')!==-1||(m.data&&m.data.indexOf('image/')!==-1);var isVideo=mime.indexOf('video/')!==-1||(m.data&&m.data.indexOf('video/')!==-1);var failed=(m.type==='file'&&m.mediaId&&m.r2Status==='failed');var canRetry=failed&&!!localSrc;var safeName=(m.name||'').replace(/"/g,'&quot;');if(failed){var retryBtn=canRetry?'<button class="sm-retry-btn" onclick="retryMediaUpload(\''+s.id+'\',\''+m.mediaId+'\')">다시 올리기</button>':'<span class="sm-failed-hint">최초 업로드 기기에서 다시 첨부 필요</span>';return '<div class="sm-failed"><div class="sm-failed-icon">⚠️</div><div class="sm-failed-text"><div class="sm-failed-title">영상 업로드 실패</div><div class="sm-failed-name">'+safeName+'</div></div>'+retryBtn+'</div>';}if(m.type==='file'&&src&&isImg) return '<img class="sm-thumb" src="'+src+'" onclick="openMediaView(this.src)" alt="'+safeName+'">';if(m.type==='file'&&src&&isVideo){var vid='v_'+s.id+'_'+mi;var onerr='onerror="handleVideoLoadError(\''+s.id+'\',\''+(m.mediaId||'')+'\',\''+vid+'\')"';return '<div class="video-wrap" id="wrap_'+vid+'"><video class="sm-video" id="'+vid+'" src="'+src+'" controls playsinline preload="auto" crossorigin="anonymous" '+onerr+'></video><div class="video-controls"><button class="vc-btn" onclick="toggleLoop(\''+vid+'\')" id="loop_'+vid+'" title="반복 재생">반복</button><span class="vc-spacer"></span><button class="vc-btn" onclick="setSpeed(\''+vid+'\',1)">1x</button><button class="vc-btn" onclick="setSpeed(\''+vid+'\',0.5)">0.5x</button><button class="vc-btn" onclick="setSpeed(\''+vid+'\',0.25)">0.25x</button><button class="vc-btn" onclick="setSpeed(\''+vid+'\',0.125)">0.125x</button><span class="vc-speed" id="spd_'+vid+'">1x</span></div></div>';}if(m.type==='file'&&!src) return '<div class="sm-missing">미디어 로딩 중</div>';return '';}).join('')+'</div>':''}
