...
function _render(){
          ...
          sessions.map((s,si) => { var so=(S.openSessions&&S.openSessions[s.id]!==undefined)?S.openSessions[s.id]:(si===0); var prev=(s.content||'').replace(/\s+/g,' ').trim(); return `
            <div class="session-card${so?' open':''}${(so&&S._animSession===s.id)?' just-opened':''}">
              ...
              <div class="session-bd">
                ...
                ${(s.rawTranscript&&s.rawTranscript.trim()&&s.rawTranscript.trim()!==(s.content||'').trim())?'<details class="raw-transcript"><summary>🎙 녹음 원문 전체 보기 ('+Math.round(s.rawTranscript.trim().length)+'자)</summary><div class="raw-transcript-body">'+s.rawTranscript.replace(/</g,'&lt;').replace(/\n/g,'<br>')+'</div></details>':''}
  ...
  <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
    <div class="modal">
      ...
      ${(S.newSession.rawTranscript&&S.newSession.rawTranscript.trim())?'<details class="raw-transcript"><summary>🎙 녹음 원문 전체 ('+S.newSession.rawTranscript.trim().length+'자) — AI 정리가 놓친 게 없나 확인</summary><div class="raw-transcript-body">'+S.newSession.rawTranscript.replace(/</g,'&lt;').replace(/\n/g,'<br>')+'</div></details>':''}
